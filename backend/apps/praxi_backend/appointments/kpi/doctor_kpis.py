"""KPIs for the doctors dashboard.

Moved 1:1 from `praxi_backend.dashboard.doctor_kpis` in Phase 2F.
...
from praxi_backend.appointments.models import Appointment, DoctorAbsence, DoctorHours
...
def _generate_demo_services(doctor_id: int) -> list[dict]:
    ...
    services = [
        ...
        {
            "code": "GOP 7",
            "name": "Vollständige Untersuchung",
            "points": 260,
            "count": random.randint(30, 100),
        },
        {
            "code": "GOP 8",
            "name": "Untersuchung eines Organsystems",
            "points": 160,
...
def calculate_doctor_utilization(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
...
def calculate_appointment_volume(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
    ...
    daily_counts = (
        appointments.filter(status__in=["completed", "confirmed", "scheduled"])
...
def calculate_no_show_rate(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
...
def calculate_cancellation_rate(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
...
def calculate_treatment_duration(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
    ...
    if not durations:
        return {"avg_planned": 0, "avg_actual": 0, "efficiency": 100, "count": 0}
...
def calculate_new_patient_rate(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
...
def calculate_peak_times(doctor: User, start_date: date, end_date: date) -> dict[str, Any]:
    ...
    heatmap_data = []
    for day_idx, day_name in enumerate(weekdays[:5]):
        row_data = []
        for hour in working_hours:
            value = matrix[day_idx][hour]
            intensity = round(value / max(1, max_val) * 100, 1)
            row_data.append({"hour": hour, "count": value, "intensity": intensity})
        heatmap_data.append({"day": day_name, "hours": row_data})
