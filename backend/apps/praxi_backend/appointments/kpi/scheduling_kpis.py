...
Moved 1:1 from `praxi_backend.dashboard.scheduling_kpis` in Phase 2F.
...
from django.db.models import Avg, Count, F, Min, Q, Sum  # noqa: F401
from django.db.models.functions import ExtractHour  # noqa: F401
from django.db.models.functions import ExtractWeekDay
from django.utils import timezone
from praxi_backend.appointments.models import DoctorBreak  # noqa: F401
...
def calculate_no_show_rate() -> dict[str, Any]:
    ...
    prev_past = Appointment.objects.using("default").filter(
        end_time__lt=prev_30_end, start_time__gte=prev_30_start
    )
...
def calculate_completion_rate() -> dict[str, Any]:
    ...
    appointments = Appointment.objects.using("default").filter(
        start_time__gte=last_30_start, start_time__lte=last_30_end
    )
    ...
    return {"rate": rate, "completed": completed, "total": total}
...
def calculate_cancellation_rate() -> dict[str, Any]:
    ...
    total = Appointment.objects.using("default").filter(created_at__gte=last_30_start).count()

    cancelled = (
        Appointment.objects.using("default")
        .filter(created_at__gte=last_30_start, status="cancelled")
        .count()
    )
...
def calculate_average_lead_time() -> dict[str, Any]:
    ...
    appointments = (
        Appointment.objects.using("default")
        .filter(created_at__gte=last_30_start, status__in=["scheduled", "confirmed", "completed"])
...
def calculate_rebooking_rate() -> dict[str, Any]:
    ...
    appointments = Appointment.objects.using("default").filter(created_at__gte=last_30_start)
...
def calculate_new_patient_conversion() -> dict[str, Any]:
    ...
    appointments = (
        Appointment.objects.using("default")
        .filter(start_time__gte=last_30_start, status__in=["scheduled", "confirmed", "completed"])
        .values("patient_id")
    )
...
def get_scheduling_trends() -> dict[str, Any]:
    ...
    for i in range(12):
        ...
        no_show = appointments.filter(
            status__in=["scheduled", "confirmed"], end_time__lt=now
        ).count()
...
def get_all_scheduling_kpis() -> dict[str, Any]:
    ...
    logger.debug("kpi.scheduling.get_all_scheduling_kpis start")
    with timed_block("kpi.scheduling.get_all_scheduling_kpis", log=logger, level="debug"):
    ...
    logger.debug("kpi.scheduling.get_all_scheduling_kpis end")
    return result
