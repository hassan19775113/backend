...
def calculate_slot_utilization() -> dict[str, Any]:
    ...
    prev_booked = Appointment.objects.using('default').filter(
...
def calculate_cancellation_rate() -> dict[str, Any]:
    ...
    now = timezone.now()
    late_cancellations = Appointment.objects.using('default').filter(
        created_at__gte=last_30_start,
        status='cancelled',
...
def calculate_scheduling_efficiency() -> dict[str, Any]:
    ...
    efficiency_score = max(0, min(100, base_score - no_show_penalty - cancellation_penalty - rebooking_penalty))
