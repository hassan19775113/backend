...
Dieses Modul enthält alle Berechnungen für Patientenstatus, Risiko-Scores,
Vitaldaten-Trends, Laborwert-Ampeln und Terminaktivität.
"""
...
def _generate_demo_diagnoses(patient_id: int) -> list[dict]:
    ...
    all_diagnoses = [
        ...
        {'icd': 'J06.9', 'name': 'Akute Infektion der oberen Atemwege', 'chronic': False, 'severity': 'low'},
        {'icd': 'M54.5', 'name': 'Kreuzschmerz', 'chronic': False, 'severity': 'low'},
        {'icd': 'E78.0', 'name': 'Hypercholesterinämie', 'chronic': True, 'severity': 'low'},
        {'icd': 'K21.0', 'name': 'Gastroösophageale Refluxkrankheit', 'chronic': True, 'severity': 'low'},
        {'icd': 'F32.1', 'name': 'Mittelgradige depressive Episode', 'chronic': False, 'severity': 'medium'},
...
def calculate_patient_status(patient_id: int) -> PatientStatus:
    ...
    Status-Kategorien:
    - active: Regulärer Patient mit Terminen in den letzten 6 Monaten
    - in_treatment: Patient mit laufender Behandlung (mehrere Termine in 30 Tagen)
    - inactive: Kein Termin in den letzten 12 Monaten
    - risk: Risikopatient basierend auf Diagnosen/Vitaldaten
    """
    ...
    recent_appointments = Appointment.objects.using('default').filter(
        patient_id=patient_id,
        start_time__gte=six_months_ago,
        status__in=['completed', 'confirmed', 'scheduled'],
    ).count()
    
    recent_30_days = Appointment.objects.using('default').filter(
        patient_id=patient_id,
        start_time__gte=thirty_days_ago,
        status__in=['completed', 'confirmed', 'scheduled'],
    ).count()
    
    has_any_recent = Appointment.objects.using('default').filter(
        patient_id=patient_id,
...
def calculate_patient_risk_score(patient_id: int) -> dict[str, Any]:
    ...
    - Vitaldaten-Trends
    - Laborwerte
    - Terminverhalten (No-Shows)
    
    Score: 0-100 (höher = höheres Risiko)
    """
    ...
    if vitals:
        ...
        if latest['bmi'] >= 35:
            score += 10
            factors.append({'name': 'BMI ≥35 (Adipositas Grad II+)', 'points': 10, 'severity': 'high'})
        elif latest['bmi'] >= 30:
            score += 5
    ...
    no_shows = Appointment.objects.using('default').filter(
        patient_id=patient_id,
        start_time__gte=year_ago,
        start_time__lt=now,
        status='scheduled',  # Nicht bestätigt/completed = No-Show
    ).count()
...
def calculate_appointment_activity(patient_id: int) -> dict[str, Any]:
    ...
    # Alle Termine des letzten Jahres
    ...
    # No-Shows: Vergangene Termine die weder completed noch cancelled sind
    no_shows = past.filter(status__in=['scheduled', 'confirmed']).exclude(
        status='completed'
    ).count()
    # Korrektur: Nur scheduled (nicht confirmed) zählen als No-Show
    no_shows = past.filter(status='scheduled').count()
    
    # Letzter Termin
    last_appointment = Appointment.objects.using('default').filter(
        patient_id=patient_id,
        start_time__lt=now,
        status='completed',
    ...
    # Nächster Termin
    next_appointment = Appointment.objects.using('default').filter(
        patient_id=patient_id,
        start_time__gte=now,
...
def calculate_medication_compliance(patient_id: int) -> dict[str, Any]:
    ...
    In einer echten Anwendung würde dies aus:
    - Rezept-Daten
    - Apotheken-Abholungen
    - Patienten-Feedback
    berechnet werden.
    """
...
def calculate_vital_trends(patient_id: int) -> dict[str, Any]:
    ...
    # Bewertung der aktuellen Werte
    def evaluate_vital(key: str, value: float, ranges: dict) -> dict:
    ...
    evaluations = {
        ...
        'temperature': evaluate_vital('temperature', current['temperature'], {'low': 36.0, 'high': 37.5}),
...
def calculate_lab_traffic_light(patient_id: int) -> dict[str, Any]:
    ...
    Kategorien:
    - green: Im Normalbereich
    - yellow: Leicht erhöht/erniedrigt
    - red: Kritisch
    """
    labs = _generate_demo_labs(patient_id)
...
def get_patient_overview_stats() -> dict[str, Any]:
    ...
    # Aktive Patienten (mit Termin in den letzten 6 Monaten)
    active_patient_ids = Appointment.objects.using('default').filter(
        start_time__gte=six_months_ago,
        status__in=['completed', 'confirmed', 'scheduled'],
    ...
    today_patients = Appointment.objects.using('default').filter(
    ...
    # Da wir kein created_at in appointments haben, schätzen wir neue Patienten
    # basierend auf erstem Termin
    new_patients_this_month = Appointment.objects.using('default').filter(
        start_time__gte=month_start,
        start_time__lt=now,
    ).values('patient_id').annotate(
        first_visit=Count('id')
    ).filter(first_visit=1).count()
...
def get_all_patient_kpis(patient_id: int) -> dict[str, Any]:
    ...
    if not profile:
        return {'error': 'Patient nicht gefunden', 'patient_id': patient_id}
